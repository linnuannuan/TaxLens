...
<script>
  ...
  export default {
    ...
    data() {
      return {
        svg: null,
        // affiliatedPartyList: this.props.affiliatedPartyList
      }
    },
    // 不需要实时更新数据，但需要初始化
    watch:{
      affiliatedPartyList: function() {
        this.renderTsne()
      },
    },
    mounted:function(){
    ...
    methods: {
      initTsne() {
        ...
        this.canvas = d3.select("#group_cluster_view").append("canvas")
            .attr("width", width)
            .attr("height", height);
      },
      renderTsne() {
    ...
    //     "Longitude": "-43.93778"
    // }];
        let tsne_data = lodash.cloneDeep(this.affiliatedPartyList.slice(0,50))
        console.log('tsne_data',tsne_data)

        // get the svg
        // /* eslint-disable */
        // console.log('tsnejs', tsnejs)
        const margin = 50, width = this.$el.clientWidth, height = this.$el.clientHeight
        ...
        // num_ap_txn: 2
        // num_nodes: 2
        const data = tsne_data
                            .sort((x,y) =>{return x.num_ap_txn <y.num_ap_txn})
                            .map((d) => [+d.num_ap_txn, +d.num_nodes])

        console.log('sort data',data)

        // 以 ap txn 数量为 r的大小
        let scale_atx_num = d3.scaleSqrt().domain([data[0][0], data[data.length-1][0]]).range([0.2, 24])
        console.log('scale min:',data[0][0], 'scale max:', data[data.length-1][0])



        // let scale_node_num = d3.scaleSqrt().domain([2, 569]).range([0.2, 24])
        ...
        .force('collide', d3.forceCollide().radius(d => 1.5 + scale_atx_num(d[0])))
        .on('tick', ()=>{
            ...
            //     debugger;
              let nodes = data.map((d) => {
                return {
                  x: d.x,
                  y: d.y,
                  //  get the r based on the num of Affiliated transaction in the group
                  r: scale_atx_num(d[0]),
                  // get the color based on the num of nodes in the group
                  // color: scale_node_num(d[1]),
                  color:color((parseInt(d[0]))%10)
                };
